...
class PaginatorSession:
    ...
    def __init__(self, ctx, timeout=60, pages=[], color=discord.Color.green(), footer='', bot=None):
    ...
    async def show_page(self, index: int):
        ...
        if self.running:
            ...
            if isinstance(self.ctx, discord.Interaction):
                await self.ctx.response.send_message(embed=page)
                ...
                async for message in self.ctx.channel.history():
                    if not message.embeds:
                        continue
                    if message.embeds[0].title == page.title and message.embeds[0].colour == page.colour:
            ...
            for reaction in self.reactions.keys():
                if len(self.pages) < 3 and reaction in '⏮⏭':
                    continue  # ignores 2 page embed first and last emojis
                if len(self.pages) < 2 and reaction in '◀▶':
                    continue  # ignored 1 page embed previous and next emojis
    ...
    def react_check(self, reaction, user):
        ...
        if (isinstance(self.ctx, discord.Interaction) and user.id != self.ctx.user.id) or (not isinstance(self.ctx, discord.Interaction) and user.id != self.ctx.author.id):
